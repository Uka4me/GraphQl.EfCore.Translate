            ...
            where TItem : class
        {
            ...
            if (keyNames is not null)
            {
                if (ArgumentReader.TryReadIds(GetArguments, out var values))
                {
                    ...
                    var predicate = ExpressionBuilderWhere<TItem>.BuildPredicate(keyName, Comparison.In, values);
                ...
                {
                    var keyName = GetKeyName(keyNames);
                    var predicate = ExpressionBuilderWhere<TItem>.BuildPredicate(keyName, Comparison.Equal, new[] { value });
                    queryable = queryable.Where(predicate);
                }
            }
            ...
            {
                var predicate = ExpressionBuilderWhere<TItem>.BuildPredicate(wheres);
                queryable = queryable.Where(predicate);
            }
